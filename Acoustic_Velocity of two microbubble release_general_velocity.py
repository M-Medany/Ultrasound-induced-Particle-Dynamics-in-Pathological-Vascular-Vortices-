# spawn_safe_mb_two_overdamped.py — two microbubbles, overdamped slip + acoustic ejection
import os
import shutil
import numpy as np
import pandas as pd
...
# Robustness toggles
SAVE_JPG_FRAMES = False
CLEAR_OUTPUT_DIR = False

# ===================== ACOUSTIC + RADIUS MODEL (NEW) =====================
# Overdamped model parameters (units serve as scales relative to CSV)
mu              = 1.0e-3      # Pa·s (effective viscosity scale)
Aac             = 5.0e4       # Pa/m  (lumped acoustic drive; tune)
GHAT            = np.array([1.0, 0.0])  # forcing direction (transducer -> opposite wall)
R0_single       = 2.5         # initial single-bubble radius (CSV length units, e.g., µm)
growth_zone_fr  = 0.30        # grow only when r_to_eye < 0.30 * a
dRdt_inside     = 0.015       # radius growth rate per time-unit (CSV units/time); tune

# Relaxation of (u,v) to overdamped target velocity
TAU_RELAX       = 1.0         # time constant (same 'time' units as integration)
...
# NEW globals for overdamped model
MU = None
AAC = None
GHATV = None
GHATN = None
...
def estimate_vortex_center(df):
    """Robust center finder for ANY units. Trims by percentiles; minimizes radial component."""
...
# ----------------- Overdamped model helpers (NEW) --------------
def unit(v):
    n = np.hypot(v[0], v[1]) + 1e-15
    return np.array([v[0]/n, v[1]/n])

def local_speed(x, y):
    """Local |u_f| from COMSOL at (x,y)."""
    uf = kNN_velocity_field(x, y, k=8)
    return float(np.hypot(uf[0], uf[1]))

def phi_index(Rc, U_loc):
    """Φ(R) = AAC * R^2 / (4.5 * MU * U)."""
    Ueff = max(U_loc, 1e-12)
    return (AAC * Rc * Rc) / (4.5 * MU * Ueff)

def overdamped_velocity(x, y, Rc):
    """
    v_MB = u_f + (- Vc * ∇p_vort + F_rad) / (6π μ Rc),
    with F_rad = AAC * Vc * GHATN.
    """
    uf    = kNN_velocity_field(x, y, k=8)
    gp    = grad_p_rankine(x, y, XC, YC, GAM, RHO, A)
    Vc    = (4.0/3.0) * np.pi * Rc**3
    Frad  = AAC * Vc * GHATN
    slip  = (-Vc * gp + Frad) / (6.0 * np.pi * MU * max(Rc, 1e-12))
    return uf + slip

def grow_radius(x, y, Rc, a_core=A, growth_zone_frac=growth_zone_fr, dRdt=dRdt_inside):
    """Grow only near the eye: r_to_eye < growth_zone_frac * a_core."""
    r_to_eye = np.hypot(x - XC, y - YC)
    if r_to_eye < growth_zone_frac * a_core:
        return dRdt
    return 0.0
...
def rhs(t, Y):
    """
    States: [x1, y1, u1, v1, x2, y2, u2, v2, R1, R2]
    Velocities relax to the overdamped target with time constant TAU_RELAX.
    """
    x1, y1, u1, v1, x2, y2, u2, v2, R1, R2 = Y

    # Target velocities from overdamped balance
    vtar1 = overdamped_velocity(x1, y1, R1)
    vtar2 = overdamped_velocity(x2, y2, R2)

    # Relaxation of (u,v) to target
    du1dt = (vtar1[0] - u1) / max(TAU_RELAX, 1e-12)
    dv1dt = (vtar1[1] - v1) / max(TAU_RELAX, 1e-12)
    du2dt = (vtar2[0] - u2) / max(TAU_RELAX, 1e-12)
    dv2dt = (vtar2[1] - v2) / max(TAU_RELAX, 1e-12)

    # Radius growth only near the eye
    dR1dt = grow_radius(x1, y1, R1, a_core=A)
    dR2dt = grow_radius(x2, y2, R2, a_core=A)

    # Kinematics
    dx1dt, dy1dt = u1, v1
    dx2dt, dy2dt = u2, v2

    return [dx1dt, dy1dt, du1dt, dv1dt, dx2dt, dy2dt, du2dt, dv2dt, dR1dt, dR2dt]
...
def init_worker(points, u_values, v_values, xc, yc, a, gamma, rho,
                ...
                xmin, xmax, ymin, ymax,
                mu_val, aac_val, ghat_tuple):
    global TREE, UVAL, VVAL, XC, YC, A, GAM, RHO, RTOL, ATOL, CHUNK_STEPS_TARGET
    global XMIN, XMAX, YMIN, YMAX
    global MU, AAC, GHATV, GHATN

    TREE = cKDTree(points)
    UVAL = u_values
    VVAL = v_values
    XC, YC = xc, yc
    A, GAM, RHO = a, gamma, rho
    RTOL, ATOL = rtol, atol
    ...
    MU = mu_val
    AAC = aac_val
    GHATV = np.array(ghat_tuple, dtype=float)
    n = np.hypot(GHATV[0], GHATV[1])
    GHATN = GHATV / (n if n > 0 else 1.0)
...
def main():
    ...
    with ctx.Pool(processes=num_chunks,
                  ...
                  initargs=(points, u_values, v_values, xc, yc, A_local,
                            ...
                            rtol, atol, chunk_steps_target,
                            xmin, xmax, ymin, ymax,
                            mu, Aac, tuple(GHAT))) as pool:
        # first chunk
        Yprev = [x0_1, y0_1, u0_1, v0_1, x0_2, y0_2, u0_2, v0_2, R0_single, R0_single]
    ...
    final_y = np.concatenate(final_y_list, axis=1)  # rows: [x1,y1,u1,v1,x2,y2,u2,v2,R1,R2]
    ...
    def save_frames():
        ...
        R1s, R2s = final_y[8], final_y[9]
        n = tx1.size

        # Precompute Φ and first ejection index for markers
        def first_eject_index(tx, ty, Rseries):
            Phi = []
            for xi, yi, Ri in zip(tx, ty, Rseries):
                Uloc = local_speed(xi, yi)
                Phi.append(phi_index(Ri, Uloc))
            Phi = np.array(Phi)
            if Phi.size > 0 and (Phi > 1.0).any():
                return int(np.argmax(Phi > 1.0)), Phi
            return None, Phi

        i_ej1, Phi1 = first_eject_index(tx1, ty1, R1s)
        i_ej2, Phi2 = first_eject_index(tx2, ty2, R2s)
        ...
        for i in range(0, n, frame_interval):
            ...
            # ejection markers (first Φ>1)
            if i_ej1 is not None and i >= i_ej1:
                ax.plot(tx1[i_ej1], ty1[i_ej1], marker='*', ms=10, color='orange',
                        label='MB1 ejection (Φ>1)' if i == i_ej1 else None)
            if i_ej2 is not None and i >= i_ej2:
                ax.plot(tx2[i_ej2], ty2[i_ej2], marker='*', ms=10, color='magenta',
                        label='MB2 ejection (Φ>1)' if i == i_ej2 else None)

            # black core circle
            core = plt.Circle((xc, yc), A_local, edgecolor=CORE_EDGE_COLOR, facecolor='none', lw=2)
            ax.add_patch(core)

            ax.set_title('Velocity field + Two MB trajectories (overdamped model)', fontsize=TITLE_FONTSIZE)
            ...
            if i == 0:
                ax.legend(loc='upper right', ncol=1, prop={'size': LEGEND_FONTSIZE})
    ...
    def save_static():
        ...
        # ejection markers
        if i_ej1 is not None:
            ax.plot(tx1[i_ej1], ty1[i_ej1], marker='*', ms=10, color='orange', label='MB1 ejection (Φ>1)')
        if i_ej2 is not None:
            ax.plot(tx2[i_ej2], ty2[i_ej2], marker='*', ms=10, color='magenta', label='MB2 ejection (Φ>1)')

        core = plt.Circle((xc, yc), A_local, edgecolor=CORE_EDGE_COLOR, facecolor='none', lw=2)
        ax.add_patch(core)

        ax.set_title('Velocity field + Two MB trajectories (static, overdamped model)', fontsize=TITLE_FONTSIZE)
        ax.set_xlabel('x (CSV units)', fontsize=LABEL_FONTSIZE)
        ax.set_ylabel('y (CSV units)', fontsize=LABEL_FONTSIZE)
        ax.tick_params(axis='both', labelsize=TICK_FONTSIZE)
        ax.set_aspect('equal', 'box'); ax.grid(alpha=0.3)
        ...
        fig.tight_layout()
        try:
            plt.savefig("velocity_Trajectory_two_mb_overdamped.png", format="png",
                        dpi=DPI_STATIC, bbox_inches="tight",
                        pil_kwargs={"compress_level": 1})
        except Exception as e:
            print(f"PNG save failed for static ({e}). Saving JPG fallback.")
            plt.savefig("velocity_Trajectory_two_mb_overdamped.jpg", format="jpg",
                        dpi=DPI_STATIC, bbox_inches="tight")
        finally:
            plt.close(fig)
        print("Saved: velocity_Trajectory_two_mb_overdamped.(png/jpg)")
...
if __name__ == "__main__":
    # Set globals used in workers via initializer
    main()
