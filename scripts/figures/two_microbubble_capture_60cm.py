...
def save_center_diagnostic(vf, roi_m, xc, yc, a):
    ...
    fig.tight_layout(); fig.savefig(REPO_ROOT / "plotting" / "diagnostic_center.png"); plt.close(fig)
...
def main():
    ...
    }).to_csv(REPO_ROOT / "outputs" / "trajectory_two_mb.csv", index=False)
    ...
    fig.tight_layout()
    (REPO_ROOT / "plotting").mkdir(exist_ok=True)
    plt.savefig(REPO_ROOT / "plotting" / "velocity_Trajectory_two_mb.png",dpi=DPI_STATIC,bbox_inches="tight",
