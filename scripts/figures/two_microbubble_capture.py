...
def main():
    ...
    traj_out = REPO_ROOT / "outputs" / "trajectory_two_mb.csv"
    ...
    def save_static():
        ...
        try:
            plt.savefig(REPO_ROOT / "plotting" / "velocity_Trajectory_two_mb.png", format="png",
                        dpi=DPI_STATIC, bbox_inches="tight",
                        pil_kwargs={"compress_level": 1})
        except Exception as e:
            print(f"PNG save failed for static ({e}). Saving SVG fallback.")
            plt.savefig(REPO_ROOT / "plotting" / "velocity_Trajectory_two_mb.svg", format="svg",
                        dpi=DPI_STATIC, bbox_inches="tight")
        finally:
            plt.close(fig)
